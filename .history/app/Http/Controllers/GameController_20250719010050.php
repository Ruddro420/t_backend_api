...
class GameController extends Controller
{
    ...
    public function getMatches()
    {
        ...
        foreach ($matches as $match) {
            $match->category_name = $match->category ? $match->category->name : 'N/A';
        }
        $matches->makeHidden(['category']);
        foreach ($matches as $match) {
            $match->rooms_count = $match->rooms->count();
        }
        // Removed redundant assignment to $match->match_id
        foreach ($matches as $match) {
            $match->title = $match->match_name . ' - ' . $match->category_name;
        }
        $matches->makeHidden(['rooms', 'category_id', 'match_name']);
        $matches->makeVisible(['match_id', 'title']);
